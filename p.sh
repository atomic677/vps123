...
start_vm() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        if [[ "${BACKGROUND_MODE:-false}" == true ]]; then
            ...
            # Remove nographic/serial if present and add proper daemonize options
            qemu_cmd+=(-daemonize -pidfile "$pid_file")

            # Replace serial mon:stdio with serial file for background mode
            local filtered_cmd=()
            ...
            for arg in "${qemu_cmd[@]}"; do
                ...
                if [[ "$arg" == "mon:stdio" ]]; then
                    continue
                fi
                filtered_cmd+=("$arg")
            done
            qemu_cmd=("${filtered_cmd[@]}")

            # Add serial output to log file
