...
setup_vm_image() {
    ...
    if [[ -f "$IMG_FILE" ]]; then
        ...
        if [[ "$IMG_URL" =~ \.(xz|gz|bz2|zip)$ ]]; then
            ...
            if [[ "$IMG_URL" =~ \.xz$ ]]; then
                xz -d -c "$temp_compressed" > "$IMG_FILE.tmp"
            elif [[ "$IMG_URL" =~ \.gz$ ]]; then
                gunzip -c "$temp_compressed" > "$IMG_FILE.tmp"
            elif [[ "$IMG_URL" =~ \.bz2$ ]]; then
                bunzip2 -c "$temp_compressed" > "$IMG_FILE.tmp"
            fi
    ...
    if load_vm_config "$vm_name"; then
        ...
        if [[ "$KVM_ENABLED" == true ]]; then
            if [ -e /dev/kvm ] && [ -r /dev/kvm ] && [ -w /dev/kvm ]; then
                ...
                print_status "WARN" "KVM requested but not available, continuing without acceleration"
            fi
        fi
    ...
    if load_vm_config "$vm_name"; then
        ...
        while true; do
            ...
            case $edit_choice in
                ...
                10)
                    while true; do
                        read -p "$(print_status "INPUT" "Enable KVM acceleration? (y/n, current: $KVM_ENABLED): ")" kvm_input
                        kvm_input="${kvm_input:-}"
...
declare -A OS_OPTIONS=(
    ...
    ["Oracle Linux 8"]="oracle|8|https://yum.oracle.com/templates/OracleLinux/OL8/u10/x86_64/OL8U10_x86_64-kvm-b245.qcow2|oracle8|oracle|oracle"
    ["Oracle Linux 9"]="oracle|9|https://yum.oracle.com/templates/OracleLinux/OL9/u5/x86_64/OL9U5_x86_64-kvm-b256.qcow2|oracle9|oracle|oracle"
    ["Kali Linux"]="kali|rolling|https://kali.download/cloud-images/current/kali-linux-2024.4-cloud-genericcloud-amd64.tar.xz|kali|kali|kali"
    ["FreeBSD 14"]="freebsd|14|https://download.freebsd.org/releases/VM-IMAGES/14.2-RELEASE/amd64/Latest/FreeBSD-14.2-RELEASE-amd64.qcow2.xz|freebsd14|freebsd|freebsd"
    ["Alpine Linux 3.20"]="alpine|3.20|https://dl-cdn.alpinelinux.org/alpine/v3.20/releases/cloud/nocloud_alpine-3.20.3-x86_64-bios-cloudinit-r0.qcow2|alpine320|alpine|alpine"
    ["Alpine Linux 3.21"]="alpine|3.21|https://dl-cdn.alpinelinux.org/alpine/v3.21/releases/cloud/nocloud_alpine-3.21.0-x86_64-bios-cloudinit-r0.qcow2|alpine321|alpine|alpine"
    ["Gentoo Linux"]="gentoo|openstack|https://distfiles.gentoo.org/releases/amd64/autobuilds/current-cloud-amd64-openstack/gentoo_openstack-amd64-default-20241215T162202Z.qcow2|gentoo|gentoo|gentoo"
    ["OpenBSD 7.6"]="openbsd|76|https://cdn.openbsd.org/pub/OpenBSD/7.6/amd64/install76.img|openbsd76|openbsd|openbsd"
    ["NetBSD 10"]="netbsd|10|https://cdn.netbsd.org/pub/NetBSD/NetBSD-10.0/images/NetBSD-10.0-amd64.img|netbsd10|netbsd|netbsd"
