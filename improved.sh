...
load_vm_config() {
    ...
    if [[ -f "$config_file" ]]; then
        ...
        unset DISK_SIZE MEMORY SWAP_SIZE CPUS SSH_PORT GUI_MODE PORT_FORWARDS IMG_FILE SEED_FILE CREATED KVM_ENABLED
...
create_new_vm() {
    ...
    while true; do
        read -p "$(print_status "INPUT" "Swap size in MB (default: 0, enter 0 to disable): ")" SWAP_SIZE
        SWAP_SIZE="${SWAP_SIZE:-0}"
        if validate_input "number" "$SWAP_SIZE"; then
            break
        fi
    done
...
setup_vm_image() {
    ...
    if ! mkdir -p "$VM_DIR"; then
        ...
        print_status "INFO" "Please ensure you have write permissions to the parent directory"
        exit 1
    fi
    
    # Verify directory is writable
    if [[ ! -w "$VM_DIR" ]]; then
        print_status "ERROR" "VM directory is not writable: $VM_DIR"
        print_status "INFO" "Please check permissions on $VM_DIR"
        exit 1
    fi
...
EOF
    ...
    # Add swap configuration if swap size is greater than 0
...
swap:
  ...
  maxsize: ${SWAP_SIZE}M
EOF
        print_status "INFO" "Swap configured: ${SWAP_SIZE}MB"
    fi
...
start_vm() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        # Set machine type based on KVM availability (must be set once, not duplicated)
        if [[ "${KVM_ENABLED:-false}" == true ]] && [[ -e /dev/kvm ]]; then
            qemu_cmd+=(-machine type=q35,accel=kvm,smm=off)
        else
            qemu_cmd+=(-machine type=q35,accel=tcg,smm=off)
        fi
...
stop_vm() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        if is_vm_running "$vm_name"; then
            ...
            # Try to get PID from pidfile first
            local pid=""
            local pid_file="$VM_DIR/$vm_name.pid"
            if [[ -f "$pid_file" ]]; then
            ...
            # If no PID from file, find it from process
            if [[ -z "$pid" ]] || ! kill -0 "$pid" 2>/dev/null; then
                pid=$(pgrep -f "qemu-system-x86_64.*$VM_NAME" | head -1)
            fi

            if [[ -n "$pid" ]]; then
                print_status "INFO" "Found QEMU process (PID: $pid)"

                # Send SIGTERM for graceful shutdown
                print_status "INFO" "Sending SIGTERM..."
                ...
                # Fallback to pkill
                print_status "INFO" "Using pkill to stop VM..."
...
declare -A OS_OPTIONS=(
    ...
    ["Alpine Linux Latest (3.23)"]="alpine|latest|https://dl-cdn.alpinelinux.org/alpine/v3.23/releases/cloud/nocloud_alpine-3.23.2-x86_64-bios-cloudinit-r0.qcow2|alpine-latest|alpine|alpine"
    ...
    ["Fedora 42"]="fedora|42|https://download.fedoraproject.org/pub/fedora/linux/releases/42/Cloud/x86_64/images/Fedora-Cloud-Base-Generic.x86_64-42-1.1.qcow2|fedora42|fedora|fedora"
    ...
    ["Parrot OS Security 7.0"]="parrot|7.0-security|https://deb.parrot.sh/direct/parrot/iso/7.0/Parrot-security-7.0_amd64.qcow2|parrot|parrot|parrot"
    ...
    ["Gentoo Linux"]="gentoo|cloudinit|https://distfiles.gentoo.org/releases/amd64/autobuilds/current-di-amd64-cloudinit/di-amd64-cloudinit-20251214T164554Z.qcow2|gentoo|gentoo|gentoo"
    ...
    ["NetBSD 10.1"]="netbsd|101|https://cdn.netbsd.org/pub/NetBSD/NetBSD-10.1/images/NetBSD-10.1-amd64-install.img.gz|netbsd101|netbsd|netbsd"
