...
# Features: 38+ Latest OS including Linux Mint, Direct console access, VM editing
...
check_kvm() {
    if [[ -e /dev/kvm ]] && [[ -r /dev/kvm ]] && [[ -w /dev/kvm ]]; then
        ...
        # Check if /dev/kvm exists but has permission issues
        if [[ -e /dev/kvm ]]; then
            echo
            print_status "INFO" "KVM device exists but needs permissions"
            read -p "$(print_status "INPUT" "Do you want to enable KVM? (yes/no): ")" enable_kvm

            if [[ "$enable_kvm" == "yes" ]]; then
                print_status "INFO" "Attempting to enable KVM..."

                # Try to add user to kvm group and fix permissions
                if sudo chmod 666 /dev/kvm 2>/dev/null; then
                    print_status "SUCCESS" "KVM permissions fixed!"
                    KVM_AVAILABLE=true
                    print_status "INFO" "KVM acceleration is now enabled"
                else
                    print_status "WARN" "Could not fix permissions. Try: sudo chmod 666 /dev/kvm"
                    print_status "INFO" "Or add your user to kvm group: sudo usermod -aG kvm $USER"
                fi
            else
                print_status "INFO" "Continuing without KVM acceleration"
            fi
        else
            echo
            print_status "INFO" "KVM module not loaded on this system"
            read -p "$(print_status "INPUT" "Do you want to try loading KVM module? (yes/no): ")" enable_kvm
            ...
            if [[ "$enable_kvm" == "yes" ]]; then
                ...
                if grep -q "Intel" /proc/cpuinfo; then
                    if sudo modprobe kvm_intel 2>/dev/null; then
                        print_status "SUCCESS" "Intel KVM module loaded!"
                        sudo chmod 666 /dev/kvm 2>/dev/null
                        KVM_AVAILABLE=true
                        print_status "SUCCESS" "KVM acceleration is now enabled"
                    else
                        print_status "ERROR" "Failed to load kvm_intel module"
                        print_status "INFO" "Your CPU may not support virtualization or it's disabled in BIOS"
                    fi
                elif grep -q "AMD" /proc/cpuinfo; then
                    if sudo modprobe kvm_amd 2>/dev/null; then
                        print_status "SUCCESS" "AMD KVM module loaded!"
                        sudo chmod 666 /dev/kvm 2>/dev/null
                        KVM_AVAILABLE=true
                        print_status "SUCCESS" "KVM acceleration is now enabled"
                    else
                        print_status "ERROR" "Failed to load kvm_amd module"
                        print_status "INFO" "Your CPU may not support virtualization or it's disabled in BIOS"
                    fi
                else
                    print_status "WARN" "Could not detect CPU vendor"
                    print_status "INFO" "Try manually: sudo modprobe kvm_intel or sudo modprobe kvm_amd"
                fi
            else
                print_status "INFO" "Continuing without KVM acceleration"
            fi
...
load_vm_config() {
    ...
    unset SHARED_FOLDERS NETWORK_MODE USE_KVM
...
runcmd:
  ...
  - sed -i 's/GRUB_CMDLINE_LINUX_DEFAULT="\(.*\)"/GRUB_CMDLINE_LINUX_DEFAULT="\1 console=tty0 console=ttyS0,115200n8"/' /etc/default/grub 2>/dev/null || true
  - sed -i 's/GRUB_CMDLINE_LINUX="\(.*\)"/GRUB_CMDLINE_LINUX="\1 console=tty0 console=ttyS0,115200n8"/' /etc/default/grub 2>/dev/null || true
...
build_qemu_console() {
    ...
    # Machine type with KVM acceleration based on VM config
    if [[ "${USE_KVM:-no}" == "yes" ]] && $KVM_AVAILABLE; then
    ...
    # Configure serial console properly for direct terminal access
    QEMU_CMD+=(-nographic)
...
edit_vm() {
    ...
    while true; do
        ...
        echo "  8) KVM Acceleration (Current: ${USE_KVM:-no})"
        ...
        case "$choice" in
            ...
            7)
                ...
                ;;
            8)
                read -p "$(print_status "INPUT" "Enable KVM acceleration? (yes/no) [${USE_KVM:-no}]: ")" new_kvm
                if [[ "$new_kvm" == "yes" ]] || [[ "$new_kvm" == "no" ]]; then
                    USE_KVM="$new_kvm"
                    print_status "SUCCESS" "KVM acceleration setting updated"
                else
                    print_status "ERROR" "Please enter 'yes' or 'no'"
                fi
                ;;
            0)
                save_vm_config
...
quick_create_vm() {
    ...
    read -p "$(print_status "INPUT" "Enable KVM acceleration for this virtual machine? (yes/no): ")" enable_vm_kvm
    if [[ "$enable_vm_kvm" == "yes" ]]; then
        USE_KVM="yes"
        print_status "INFO" "KVM acceleration will be enabled for this VM"
    else
        USE_KVM="no"
        print_status "INFO" "VM will run without KVM acceleration"
    fi
...
# ============================================
# OS Options - ALL LATEST VERSIONS (38+)
# ============================================
...
declare -A OS_OPTIONS=(
    ...
    ["Linux Mint 22 (Wilma)"]="mint|22|wilma|https://cloud-images.ubuntu.com/noble/current/noble-server-cloudimg-amd64.img|mint22|mint|mint"
    ["Linux Mint 21.3 (Virginia)"]="mint|21.3|virginia|https://cloud-images.ubuntu.com/jammy/current/jammy-server-cloudimg-amd64.img|mint213|mint|mint"
    ["Linux Mint 20.3 (Una)"]="mint|20.3|una|https://cloud-images.ubuntu.com/focal/current/focal-server-cloudimg-amd64.img|mint203|mint|mint"
